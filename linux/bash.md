...
```
--文件级别--
只需要加到主程序文件，调用的文件都会生效【比如在主文件里"sh xyz.sh"，xyz.sh也会生效】
#!/bin/bash
set -e // 启用 "错误终止" 选项，任何错误会退出。
set +e // 忽略错误
...
### 调试
* 暂停到输入状态，输入值到变量key: read -p "Press any key to continue..." key
* 输出详细脚本信息
```
set -x
调试脚本
...
* 所有操作尽量用绝对路径，删除必须是绝对路径[减少删除整个硬盘风险]
